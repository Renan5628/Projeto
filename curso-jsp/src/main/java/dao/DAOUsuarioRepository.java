...
public class DAOUsuarioRepository {
	...
	public ModelLogin gravarUsuario(ModelLogin objeto, Long userLogado) throws Exception {
		...
		connection.commit();
			...
			return this.consultaUsuarioLogado(objeto.getLogin());
		
		} else if (this.validarLogin(objeto.getLogin())){
			
			String sql = "UPDATE model_login SET senha=?, nome=?, email=?, perfil=?, sexo=?, cep=?, logradouro=?, bairro =?, localidade=?, uf=?, numero =?, datanascimento =?, rendamensal =? WHERE id =  "+objeto.getId()+";";
			...
			prepareSql.setString(4, objeto.getPerfil());
			prepareSql.setString(5, objeto.getSexo());
			...
			return this.consultaUsuarioID(objeto.getId());
	...
	public boolean validarLoginID(String login, Long id) throws Exception {
		String sql = "select count(1) > 0 as existe from model_login where upper(login) = upper('"+login+"') and id =" + id;
